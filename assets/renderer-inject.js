((cssText, customCssText, artDataUrl, qqArtDataUrl, petDataUrl, retroFrameDataUrl, qqAvatarDataUrl, coughAudioDataUrl, deepThemeAssets, themeConfig, qqThemeConfig, libraryThemes) => {
  ...
  const renderUsageSnapshot = () => {
    ...
    setTextContent(parts.todayActivity,
      `今日活跃度 ${formatTokenCount(visibleUsageTokens(totals.today))} / ${formatTokenCount(snapshot.growth?.dailyTokenTarget || 2_000_000)}，已加速 ${formatGrowthDays(snapshot.growth?.todayBonus)} 天`);
