...
顶部固定为 `原生 / QQ 经典 / 更多`。`QQ 经典` 合并原来的浅色、深色入口；
点击 `更多` 选择爱奇艺、哔哩哔哩、抖音、快手、腾讯视频、优酷和芒果 TV 七组平台主题。
八组主题均包含浅色与深色，跟随 **Codex 设置 → 外观 → 系统 / 浅色 / 深色**，
皮肤不再强制修改 Codex 的外观设置。系统模式以 Codex 当前解析出的明暗为准。
经典浅色保留蓝银风格，经典深色保留手机 QQ 2008–2010 风格；`原生` 即时移除皮肤。
更多菜单不再展示大黄蜂与霓虹雨夜自定义皮肤。
菜单展示色块和当前选中项，支持方向键、Home/End、Enter、Esc 和外部点击关闭。
最近选择的主题保存在本机；旧的 `light` / `dark` 选择自动迁移为 `classic`。
切换外观不改变主题选择，重新启动皮肤引擎后继续使用。

浅色配置在 `assets/theme.json`，深色配置在 `assets/theme-dark.json`。
`assets/qq-skin.css` 保留原有布局和浅色样式，`assets/qq-dark.css` 仅覆盖深色模式。
经典深浅色属于同一内置 QQ 模式，不会替换用户的自定义皮肤。此功能随源码引擎安装，
尚未包含在上方链接的 2.6.3 Release EXE 中。

爱奇艺主题配置在 `assets/theme-iqiyi.json`，其他平台配置在 `assets/themes-media.json`，
相反明暗配色在 `assets/theme-appearances.json`，共用组件覆盖 `assets/qq-media.css`。
每个平台复用原来的字体、圆角和组件结构；浅色使用中性浅底与深色品牌文字，
深色使用分层暗底与亮色强调，普通正文不随品牌变色。手机与平板官方预览用于
提炼信息层级，电脑网页 Computed CSS 用于校准材质与形状，不照搬媒体页面布局。
...
其他平台以 2026-09-22 官网及官方手机、平板预览补充复核，具体采样与设计调整见
...
QQ 模式右侧的“个人资料”显示昵称、头像、状态、签名和本机 Token 统计，与保留原生内容的“chat详情”并列。两个窗口可以分别收起，整列支持滚动。等级复用活跃成长规则：每天基础 1 天，含缓存 Token 达到 10K / 100K / 500K / 2M / 10M，分别加速 0.25 / 0.5 / 0.75 / 1 / 2 天，只取最高档。达到 Lv.N 累计需要 N² + 4N 天；进度条显示本级已积累天数 / 本级所需天数，达到新等级时归零。今日活跃度显示今日总 Token / 下一加速档位门槛及今日加速天数。

点击齿轮或底栏昵称编辑个人资料；点击头像选择经典头像，40px 与底栏 16px 版本配对切换。底栏签名支持点击编辑，Enter/失焦保存，Escape 取消。资料保存在本机皮肤设置，不修改 Codex 账号。

统计只读取当前 Windows 用户目录下的 Codex 本地 session 日志，增量缓存保存在 `%APPDATA%\CodexQQSkin\usage`。Token 展示与加速档位均包含缓存。不需要额外登录，不读取 API Key，也不会上传 prompt 或 token 数据。统计是本机口径，不是 OpenAI 官方账单或账号云端等级。
