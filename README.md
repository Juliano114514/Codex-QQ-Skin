...
| 当前支持 Codex 版本 | 26.917.6896.0（Windows 当前源码实测） |
| --- | --- |

## 安装与新版 Codex 兼容

- 修复 Windows 引擎识别和同版本重装，并补充从源码安装的说明。
- 适配新版 Codex 的窗口容器、侧栏和新建任务页；修复内嵌浏览器遮挡、侧栏圆点错位及首页标题被挤到卡片下方的问题。

## QQ 经典与个人资料

- 补齐经典 QQ 头像、个人资料和深色外观，保留昵称、签名及在线状态的本机设置。
- 将浅色、深色合并为“QQ 经典”入口；主题明暗跟随 Codex 原生外观设置。

## 平台主题

- 在“更多”中提供爱奇艺、哔哩哔哩、抖音、快手、腾讯视频、优酷和芒果 TV 七组主题。
- 为各平台建立差异化界面样式和浅色、深色配色，同时保留 Codex 原有控件与交互。

## 原生界面与性能

- 保留原生侧栏及面板选项卡，修复固定标题间距和 Mermaid 阅读宽度。
- 减少资料刷新、聊天更新及辅助窗口重复注入带来的额外开销；原生模式下暂停皮肤工作。

## 统计、成长与提示音

- 恢复活跃天数成长及每日 Token 加速档位，并从 Codex 原生缓存读取剩余额度。
- 更新任务完成提示音。
