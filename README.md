...
- 等级复用活跃成长规则：每天基础 1 天，含缓存 Token 达到 10K / 100K / 500K / 2M / 10M，分别加速 0.25 / 0.5 / 0.75 / 1 / 2 天，只取最高档。Lv.N 累计需 N² + 4N 天，以星星、月亮、太阳、皇冠显示；进度条显示本级已积累天数 / 本级所需天数，升级时归零。
- “个人资料”和包含原生环境信息、来源的“chat详情”并列显示，可分别收起，超出右栏高度后滚动。
- 齿轮或底栏昵称打开 QQ 风格个人资料页，可设置昵称、签名和在线状态；点击头像可从 254 个经典头像中选择。头像库采用用户提供的 bilibili1950835 整理素材，保留 40px / 16px 原图配对，仅在本机使用。
- 底栏签名单击编辑，Enter 或失焦保存，Escape 取消。资料和头像保存在本机皮肤设置，不修改 Codex 账号资料。
- 统计直接读取本机 `~/.codex/sessions` 与 `archived_sessions`，使用增量缓存；不需要额外账号，不读取 API Key，不上传 prompt 或 token 数据。
- Token 统计与趋势均含缓存；今日活跃度显示今日总 Token / 下一加速档位门槛及今日加速天数。fork、子 Agent 继承历史和重复 token 事件会去重。
...
- 上传过多套自定义皮肤后：在 **macOS App → 皮肤库** 里按行管理（应用 / 重命名 / 删除）；Codex 顶栏仅保留原版、浅色和深色三个入口。
...
当前源码的右上角提供四个选项（已发布的旧版安装器仍显示三个）：

- `原生`：完整恢复 Codex 官方界面。
- `浅色`：保留原有经典蓝银皮肤，使用独立浅色配置。
- `深色`：手机 QQ 2008–2010 风格，藏蓝内容区、青蓝选中态和带高光的渐变工具栏。
- `自定义`：在 Codex 原生界面结构上应用用户图片；人物与普通照片在新建任务页优先完整显示，超宽画面按照视觉焦点和安全区铺满；只有成功导入图片后才可选择。

自定义图片不会覆盖 QQ 皮肤。各选项可以随时切换，QQ 的浅色／深色选择也会保存在本机。
浅色配置位于 `assets/theme.json`；深色配置和样式位于 `assets/theme-dark.json`、
`assets/qq-dark.css`，复用现有的布局、图标和交互。
